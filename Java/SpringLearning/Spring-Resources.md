...
当Spring应用需要进行资源访问时，实际上并不需要直接使用`Resource`实现类，而是调用`ResourceLoader`实例的`getResource()`方法来获得资源，`ReosurceLoader`**将会负责选择**`Reosurce`实现类，也就是确定具体的资源访问策略，从而将应用程序和具体的资源访问策略分离开来。
